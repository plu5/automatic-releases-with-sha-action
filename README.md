...
4. **Changelog to previous semantic version tag:** Different behaviour with the parameter `automatic_release_tag`. In the original action if this parameter is provided, the changelog is generated between the given tag and the previous release with the same tag, resulting in an empty changelog if this is the first release with the given tag. I have changed it to generate the changelog between the given tag and previous tag, rather than the same tag: for example, between v1.0.0 and v1.0.1, rather than v1.0.1 and the previous v1.0.1 release. To get the old behaviour, use `is_tag_static: true`. Without that parameter set, the tag must conform to semantic versioning or the action will fail with "Error: Cannot find previous release tag because the tag does not appear to conform to semantic versioning."
